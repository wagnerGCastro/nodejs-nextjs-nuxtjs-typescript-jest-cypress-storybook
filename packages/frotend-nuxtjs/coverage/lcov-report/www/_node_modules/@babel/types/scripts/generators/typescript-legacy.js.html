...
<head>
    <title>Code coverage report for www/_node_modules/@babel/types/scripts/generators/typescript-legacy.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../../../../index.html">All files</a> / <a href="index.html">www/_node_modules/@babel/types/scripts/generators</a> typescript-legacy.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import t from "../../lib/index.js";
import stringifyValidator from "../utils/stringifyValidator.js";
import toFunctionName from "../utils/toFunctionName.js";
&nbsp;
let code = <span class="cstat-no" title="statement not covered" >`// NOTE: This file is autogenerated. Do not modify.</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >for (const type in t.NODE_FIELDS) {</span>
  const fields = <span class="cstat-no" title="statement not covered" >t.NODE_FIELDS[type];</span>
  const fieldNames = <span class="cstat-no" title="statement not covered" >sortFieldNames(Object.keys(t.NODE_FIELDS[type]), type);</span>
  const builderNames = <span class="cstat-no" title="statement not covered" >t.BUILDER_KEYS[type];</span>
&nbsp;
  const struct = <span class="cstat-no" title="statement not covered" >['type: "' + type + '";'];</span>
  const args = <span class="cstat-no" title="statement not covered" >[];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  fieldNames.forEach(<span class="fstat-no" title="function not covered" >fi</span>eldName =&gt; {</span>
    const field = <span class="cstat-no" title="statement not covered" >fields[fieldName];</span>
    ...
    let typeAnnotation = <span class="cstat-no" title="statement not covered" >stringifyValidator(field.validate, "");</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (isNullable(field) &amp;&amp; !hasDefault(field)) {</span>
<span class="cstat-no" title="statement not covered" >      typeAnnotation += " | null";</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (builderNames.includes(fieldName)) {</span>
<span class="cstat-no" title="statement not covered" >      if (areAllRemainingFieldsNullable(fieldName, builderNames, fields)) {</span>
...
&nbsp;
    const alphaNumeric = <span class="cstat-no" title="statement not covered" >/^\w+$/;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (t.isValidIdentifier(fieldName) || alphaNumeric.test(fieldName)) {</span>
<span class="cstat-no" title="statement not covered" >      struct.push(`${fieldName}: ${typeAnnotation};`);</span>
    } else {
<span class="cstat-no" title="statement not covered" >      struct.push(`"${fieldName}": ${typeAnnotation};`);</span>
    }
  });
&nbsp;
<span class="cstat-no" title="statement not covered" >  code += `export interface ${type} extends BaseNode {</span>
  ${struct.join("\n  ").trim()}
}\n\n`;
&nbsp;
  // super and import are reserved words in JavaScript
<span class="cstat-no" title="statement not covered" >  if (type !== "Super" &amp;&amp; type !== "Import") {</span>
<span class="cstat-no" title="statement not covered" >    lines.push(</span>
      `export function ${toFunctionName(type)}(${args.join(", ")}): ${type};`
    );
  } else {
    const functionName = <span class="cstat-no" title="statement not covered" >toFunctionName(type);</span>
<span class="cstat-no" title="statement not covered" >    lines.push(</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >for (const typeName of t.TYPES) {</span>
  const isDeprecated = <span class="cstat-no" title="statement not covered" >!!t.DEPRECATED_KEYS[typeName];</span>
  const realName = <span class="cstat-no" title="statement not covered" >isDeprecated ? t.DEPRECATED_KEYS[typeName] : typeName;</span>
&nbsp;
  const result =
<span class="cstat-no" title="statement not covered" >    t.NODE_FIELDS[realName] || t.FLIPPED_ALIAS_KEYS[realName]</span>
      ? `node is ${realName}`
      : "boolean";
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (isDeprecated) {</span>
<span class="cstat-no" title="statement not covered" >    lines.push(`/** @deprecated Use \`is${realName}\` */`);</span>
  }
<span class="cstat-no" title="statement not covered" >  lines.push(</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (isDeprecated) {</span>
<span class="cstat-no" title="statement not covered" >    lines.push(`/** @deprecated Use \`assert${realName}\` */`);</span>
  }
<span class="cstat-no" title="statement not covered" >  lines.push(</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >for (const type in t.DEPRECATED_KEYS) {</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >for (const type in t.FLIPPED_ALIAS_KEYS) {</span>
  const types = <span class="cstat-no" title="statement not covered" >t.FLIPPED_ALIAS_KEYS[type];</span>
<span class="cstat-no" title="statement not covered" >  code += `export type ${type} = ${types</span>
    .map(<span class="fstat-no" title="function not covered" >ty</span>pe =&gt; <span class="cstat-no" title="statement not covered" >`${type}`)</span>
    .join(" | ")};\n`;
}
<span class="cstat-no" title="statement not covered" >code += "\n";</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >code += "export interface Aliases {\n";</span>
<span class="cstat-no" title="statement not covered" >for (const type in t.FLIPPED_ALIAS_KEYS) {</span>
<span class="cstat-no" title="statement not covered" >  code += `  ${type}: ${type};\n`;</span>
}
<span class="cstat-no" title="statement not covered" >code += "}\n\n";</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >code += lines.join("\n") + "\n";</span>
&nbsp;
//
&nbsp;
<span class="cstat-no" title="statement not covered" >process.stdout.write(code);</span>
&nbsp;
//
&nbsp;
function <span class="fstat-no" title="function not covered" >areAllRemainingFieldsNullable(</span>fieldName, fieldNames, fields) {
  const index = <span class="cstat-no" title="statement not covered" >fieldNames.indexOf(fieldName);</span>
<span class="cstat-no" title="statement not covered" >  return fieldNames.slice(index).every(<span class="fstat-no" title="function not covered" >_ </span>=&gt; <span class="cstat-no" title="statement not covered" >isNullable(fields[_]))</span>;</span>
}
&nbsp;
function <span class="fstat-no" title="function not covered" >hasDefault(</span>field) {
<span class="cstat-no" title="statement not covered" >  return field.default != null;</span>
}
&nbsp;
function <span class="fstat-no" title="function not covered" >isNullable(</span>field) {
<span class="cstat-no" title="statement not covered" >  return field.optional || hasDefault(field);</span>
}
&nbsp;
function <span class="fstat-no" title="function not covered" >sortFieldNames(</span>fields, type) {
<span class="cstat-no" title="statement not covered" >  return fields.sort(<span class="fstat-no" title="function not covered" >(f</span>ieldA, fieldB) =&gt; {</span>
    const indexA = <span class="cstat-no" title="statement not covered" >t.BUILDER_KEYS[type].indexOf(fieldA);</span>
    const indexB = <span class="cstat-no" title="statement not covered" >t.BUILDER_KEYS[type].indexOf(fieldB);</span>
<span class="cstat-no" title="statement not covered" >    if (indexA === indexB) <span class="cstat-no" title="statement not covered" >return fieldA &lt; fieldB ? -1 : 1;</span></span>
<span class="cstat-no" title="statement not covered" >    if (indexA === -1) <span class="cstat-no" title="statement not covered" >return 1;</span></span>
<span class="cstat-no" title="statement not covered" >    if (indexB === -1) <span class="cstat-no" title="statement not covered" >return -1;</span></span>
